...
A from-scratch option pricing engine in Python: analytical **Black-Scholes**, **binomial trees** (CRR), and **Monte Carlo** simulation with variance reduction for exotic options plus the **Greeks**, **implied volatility**, and a reconstructed **3D volatility surface** from real market data. Every result is cross-validated three ways internally and against **QuantLib**.
...
On top of pricing, the engine inverts market prices into implied volatilities and reconstructs a full implied volatility surface from a live AAPL option chain.
...
bash
jupyter notebook   # then open notebooks/00_fondations.ipynb and run all cells
...
print(BinomialTree(100, 100, 0.05, 0.20, 1.0, n_steps=1000).price("put", "american"))

# Arithmetic Asian option (Monte Carlo)
print(mc_asian(100, 100, 0.05, 0.20, 1.0, average="arithmetic"))

Run the tests:

bash
pytest -v          # 44 tests; QuantLib tests are skipped if QuantLib isn't installed

Run the benchmark (execution times used in the report):
